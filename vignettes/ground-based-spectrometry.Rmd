...
A diagram representing the workflow and the components required to derive radiometric quantities from ground-based spectrometry measurements (using a OO-USB2000) is shown below. Green nodes represent 'constant' metadata for the instrument and reference panel i.e, this information is in frequently updated, for example after a new calibration or change in the system. Blue nodes represent metadata for a specific measurement, this information may change for each measurement. Grey nodes represent spectral 'units', with Digital Number counts (DN) being the usual starting point for radiometric conversion.

```{r workflow_diagram}
require(DiagrammeR)
grViz(paste0(file.path,"/workflow.DOT"))
```


### Instrument-specific properties
Conversion of instrument Digital Number counts to radiometric units requires instrument-specific metadata including;
...
+ The 'diameter' (*d*, m) and 'full angle' or 'angular field of view' ($\theta$~fov~, °)  of the instrument light collection apature. For the OO-USB2000 these are often properties of the fibre optic that guides light into the detector array.
...
The normalised 'instrument-specific response function' (*IRF*~norm,$\lambda$~, J s nm count^-1^) is calculated as:

$${IRF}_{\mathrm{norm, \lambda}} = \frac{t \times d_{\lambda}}{{IRF}_{\lambda}}$$

where, *t* is the integration time of the calibration measurement.
...
### Radiant energy (*Q*) [J] 
Using the normalised instrument-specific response function (*IRF*~$norm,\lambda$~, J s nm count^-1^), radiant energy is calculated as:

$$Q_{\mathrm{e}} = {{DN}_{\mathrm{norm, \lambda}}}\times{{IRF}_{norm, \lambda}}$$
